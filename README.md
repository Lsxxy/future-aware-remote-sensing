...
| ------------------------ | ------------------ | ---------------------------- | -------------- | ------------ | --------- |
| Original（未微调）       | 0.077              | 0.003                        | 0.474          | 0.423        | 34.03     |
| LoRA                     | 0.216              | 0.428                        | 0.650          | 0.388        | 43.70     |
| P-tuning                 | 0.076              | 0.002                        | 0.477          | 0.433        | 34.60     |
...
### 1. 智能图像描述（Image Captioning）

上传遥感图像（支持 TIF / PNG / JPG），AI 模型自动生成详细的文本描述。

- 支持大尺寸、高分辨率专业遥感图像
- 生成描述涵盖整体场景和关键区域细节

### 2. 多模态视觉问答（Visual Question Answering）

上传遥感图像并输入选择题（JSON 格式），系统结合图像内容推理出答案。

- 支持对象属性识别、空间关系判断、逻辑推理等复杂任务
- 结构化 JSON 输入，统一处理多种问答任务
